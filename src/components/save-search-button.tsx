          ...
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            {done ? (
              <div className="px-5 py-8 text-center">
                ...
                <p className="font-semibold text-gray-900">저장됐어요!</p>
                <p className="mt-1 text-xs text-gray-500">이 조건에 맞는 새 항목이 등록되면 알림으로 알려드려요</p>
              </div>
            ) : (
              <>
                <div className="px-5 py-5">
                  <label className="mb-1.5 block text-sm font-medium text-gray-700">이름 (선택)</label>
                  ...
                  <div className="mt-4 rounded-xl bg-gray-50 px-3 py-2.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {Object.entries(filters).filter(([_, v]) => v !== null && v !== undefined && v !== '' && v !== false).map(([k, v]) => (
                        <span key={k} className="inline-flex items-center gap-1 rounded-md bg-white border border-gray-200 px-2 py-0.5 text-[11px] font-medium text-gray-700">
