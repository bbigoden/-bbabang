...
function buildDetails(p: ParsedListing, src = ''): string {
  ...
  condBits.push(p.premium === '없음'
    ? 골라쓰기([
      ...
      '무권리로 나와 있어 들어가는 돈이 그만큼 줄어듭니다',
      '권리금 없이 시작하실 수 있는 자리입니다',
    ], seed, 6)
    : p.premium ? 골라쓰기([
      `권리금은 ${p.premium} 조건입니다`,
      `권리금은 ${p.premium}로 안내드리고 있습니다`,
      `권리금 조건은 ${p.premium}입니다`,
    ], seed, 6)
      : '권리금 조건은 확인 후 안내드리겠습니다')
  if (p.moveIn) {
    const 입주 = p.moveIn === '협의' ? '일정 협의가 가능' : p.moveIn
    condBits.push(골라쓰기([
      `입주는 ${입주} 조건입니다`,
      `입주 시기는 ${조사(입주, '으로', '로')} 잡으시면 됩니다`,
      `입주는 ${조사(입주, '으로', '로')} 협의하고 있습니다`,
    ], seed, 7))
  }
  condBits.push(골라쓰기([
    '세부 임대 조건은 협의 범위가 있으니 편하게 문의해 주세요',
    '조건은 임대인과 협의할 여지가 있으니 원하시는 선을 말씀해 주세요',
    '계약 기간과 렌트프리는 조율이 가능한 부분이라 상담 때 함께 봐 드립니다',
    '자세한 조건은 전화 주시면 임대인 입장까지 정리해서 알려드리겠습니다',
  ], seed, 2))
