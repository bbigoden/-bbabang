 ...
 *
 * **결과물에서 빼는 것이 아니다.** 이 생성기는 원문 문장을 옆기지 않고 정해진
 * 어휘로 새로 쓰므로, 애초에 옆겨질 일이 없다. 사장님이 뱅크 원문을 손볼 때
 * 참고하시라고 알리는 것뿐이다.
 *
 * **지금은 비어 둔다.** 최상급·거리·시간 표현에 이어 `성업 중`·`검증된 매출`
 * 까지 사장님이 "이 정도는 써도 된다" 로 정하셨다(2026-09-07). 중개사
 * 자격으로 감당하시는 부분이다.
 *
 * 되살리려면 여기에 다시 넣으면 된다 — 알리는 쪽만 바뀌고 글은 변하지 않는다.
 *   [/성업\s*중/, '성업 중'], [/잘\s*되는\s*자리/, '잘 되는 자리'],
 *   [/검증된?\s*매출/, '검증된 매출'], [/매출\s*보장/, '매출 보장'],
 */
...
const CONCERNS: Record<Category, string[]> = {
  office: [
    ...
    '사무실은 임대료 못지않게 관리비와 주차 조건이 매달 부담으로 남습니다. 계약 전에 항목을 나눠 확인해 두는 편이 좋습니다.',
    '직원 수가 늘거나 줄면 필요한 평수도 달라집니다. 계약 기간을 정하실 때 그 부분까지 생각해 두시면 나중이 편합니다.',
    '사무 공간은 냉난방 방식과 전기 용량에 따라 매달 드는 비용이 갈립니다. 도면만으로는 잘 드러나지 않는 부분입니다.',
  ],
  food: [
    ...
    '음식점은 환기와 냄새 문제로 이웃 점포와 부딪히는 일이 있습니다. 건물 구조와 덕트 경로를 미리 확인해 두는 편이 좋습니다.',
    '주방을 어디에 두느냐로 동선이 갈립니다. 같은 평수라도 손님 자리 수가 달라집니다.',
    '요식업은 인허가 요건이 업종마다 다릅니다. 계획하신 메뉴가 이 자리에서 가능한지 먼저 확인해야 합니다.',
  ],
  academy: [
    ...
    '학원은 정원과 강의실 면적 기준이 정해져 있습니다. 원하시는 규모가 이 자리에서 가능한지 먼저 따져봐야 합니다.',
    '수업 시간대에 주변이 얼마나 붐비는지도 봐야 합니다. 등하원 시간의 차량 흐름은 낮에 와서는 잘 보이지 않습니다.',
    '교습 시설은 소방과 피난 통로 요건을 갖춰야 합니다. 층과 구조에 따라 공사 범위가 달라집니다.',
  ],
  beauty: [
    ...
    '미용업은 급배수와 전기 용량이 맞아야 설비를 옮길 수 있습니다. 자리를 정하기 전에 확인해 두면 공사가 수월합니다.',
    '샵 자리는 간판이 보이는 각도와 출입문 위치가 예약으로 이어집니다. 도면보다 현장에서 드러나는 부분입니다.',
    '시술 공간은 칸을 어떻게 나누느냐로 손님 동선이 갈립니다. 평면이 반듯한 자리가 공사비를 줄여 줍니다.',
  ],
  large: [
    ...
    '넓은 자리는 냉난방과 관리비 부담이 함께 커집니다. 실제 쓰실 면적과 비용을 같이 놓고 보셔야 합니다.',
    '큰 평수는 드나드는 동선과 화물 승강기 여부가 운영을 좌우합니다. 면적만 보고 정하기 어려운 이유입니다.',
    '한 층을 통으로 쓰는 자리는 임대인 조건이 까다로운 경우가 있습니다. 계약 조건을 미리 맞춰 보는 편이 좋습니다.',
  ],
  residential: [
    ...
    '주택은 관리 주체가 없는 경우가 많아 수리 부담을 누가 지는지가 중요합니다. 계약서에 적어 두면 나중에 다툼이 줄어듭니다.',
    '방 개수보다 실제 쓰는 구조가 생활에 영향을 줍니다. 환기와 소음은 시간대를 바꿔 보시는 편이 정확합니다.',
  ],
  land: [
    ...
    '토지는 경계와 실제 사용 현황이 다른 경우가 있습니다. 측량 여부를 확인해 두면 뒤탈이 적습니다.',
    '지목과 실제 이용 상황이 다르면 계획하신 사업이 막힐 수 있습니다. 인허가 가능 범위를 먼저 확인해야 합니다.',
  ],
  industrial: [
    ...
    '공장은 폐수와 소음 기준이 업종마다 다릅니다. 하시려는 공정이 이 자리에서 가능한지 먼저 확인해야 합니다.',
    '창고는 지게차 동선과 상하차 공간이 실제 효율을 좌우합니다. 면적이 맞아도 마당이 좁으면 쓰기 어렵습니다.',
  ],
  retail: [
    ...
    '상가는 업종에 따라 필요한 설비가 다릅니다. 전기 용량과 급배수가 맞는지 미리 봐 두면 공사가 수월합니다.',
    '계약 기간과 임대료 인상 조건은 나중에 부담으로 돌아옵니다. 처음에 짚어 두는 편이 마음이 편합니다.',
    '보증금과 월세만 보고 정하기 어려운 것이 상가입니다. 관리 방식과 실제 쓰는 면적까지 봐야 계산이 맞습니다.',
  ],
}
...
function buildTitles(p: ParsedListing, src = ''): string[] {
  ...
  return [
    ...
    //
    // 부르는 이름과 업종이 어긋나면 안 된다. 뱅크 종류가 '상가점포' 여도
    // 사장님이 `창고 공장 분할 합병 가능` 이라 적어 둔 물건이 있다. 그걸
    // `[… 점포 월세] … 제조 추천` 이라 부르면 말이 맞지 않는다.
    join(`[${region} ${/제조|물류|창고/.test(업종 ?? '') ? '창고' : KIND_ALT[p.category]} ${deal}]`,
      자리 ?? 설비, 층, 평수대, `${업종 ?? uses[0]} 추천`, 가격),
...
/**
 * 이 매물만의 번호. 문장을 돌려 쓰기 위한 기준이다.
 *
 * 같은 매물은 몇 번을 다시 만들어도 같은 문장이 나와야 한다 — 글을 고칠 때마다
 * 문구가 바뀌면 무엇이 달라졌는지 알 수 없다. 그래서 무작위가 아니라 **매물이
 * 가진 값**(동·면적·가격)에서 뽑는다.
 */
function seedOf(p: ParsedListing): number {
  const 재료 = [p.dong, p.exclusiveArea, p.supplyArea, p.deposit, p.monthlyRent, p.salePrice, p.floor]
    .filter(Boolean).join('|')
  let h = 0
  for (let i = 0; i < 재료.length; i++) h = (h * 31 + 재료.charCodeAt(i)) % 100003
  return h
}
 ...
 * 여러 문장 중 하나를 고른다.
 *
 * 안내 문장을 한 벌로 두면 매물이 열 건이든 백 건이든 **같은 줄이 그대로**
 * 나간다. 실제로 하루치 10건이 임대 조건·문답에서 절반씩 겹쳤다. 네이버는
 * 같은 문장이 반복되는 글을 유사문서로 보고, 사람 눈에도 복붙으로 읽힌다.
 */
function 골라쓰기<T>(pool: T[], seed: number, 자리 = 0): T {
  // **자리마다 완전히 다른 값으로 흩는다.**
  //
  // 처음에는 `(seed + 자리 * 7) % 길이` 로 골랐다. 자리마다 같은 간격을 더하는
  // 것이라, 씨앗이 벌 길이의 배수만큼 차이 나는 두 매물은 **모든 자리에서**
  // 같은 문장을 골랐다. 벌이 3~4개뿐이라 그런 짝이 자주 생겼고, 같은 동·같은
  // 조건의 열 건을 나란히 놓으니 열 짝이 40% 넘게 겹쳤다.
  //
  // 자리와 씨앗을 섞어 흩으면 한 자리가 같아도 다음 자리는 따로 논다.
  //
  // `>>> 0` 을 끝까지 붙인다. 자바스크립트의 `^` 는 결과를 **부호 있는** 32비트로
  // 돌려줘서 음수가 나오고, 음수를 나머지 연산하면 음수라 `pool[-2]` 가 된다.
  // 그러면 문장 자리에 `undefined` 가 그대로 박힌다 — 실제로 그렇게 나갔다.
  let h = (seed ^ Math.imul(자리 + 1, 0x9E3779B1)) >>> 0
...
function buildIntro(p: ParsedListing, src: string): string {
  ...
  const 인사 = [
    ...
    `${소개지역} ${kindLabel} 자리를 함께 찾아 드리는 플러스불당공인중개사사무소입니다.`,
    `안녕하세요. ${소개지역}에서 ${kindLabel} 매물을 다루는 플러스불당공인중개사사무소입니다.`,
    `${소개지역} ${kindLabel} 자리를 놓고 상담드리고 있는 플러스불당공인중개사사무소입니다.`,
    `${소개지역} ${kindLabel} 매물을 정리해 올리는 플러스불당공인중개사사무소입니다.`,
  ]
...
function fitSentence(p: ParsedListing, src: string): string {
  // **갈래마다 후보를 둔다.** 예전에는 조건마다 문장이 하나뿐이라, 1층이라는
  // 이유만으로 83건이 같은 줄을 달고 나갔다. 조건이 같아도 문장은 갈라져야
  // 한다 — 뽑는 기준은 매물마다 다른 값(원문·층·면적)이라 매번 같은 것이 나온다.
  const seed = `${src}|${p.floor ?? ''}|${Math.round(mainAreaM2(p) ?? 0)}`
  ...
  // 업종 이름은 받침이 있기도 없기도 해서, 조사가 붙지 않는 형태로만 쓴다.
  if (업종) return 고르기([
    `${업종} 자리를 찾고 계신 분께 특히 잘 맞는 매물입니다.`,
    `${업종} 쪽으로 알아보고 계신 분께 맞춰 보실 만한 자리입니다.`,
    `${업종} 용도로 보고 계신다면 조건을 견줘 보실 만합니다.`,
  ])
  ...
  if (isIndustrialCat(p.category)) {
    return (mainAreaM2(p) ?? 0) >= 500
      ? 고르기([
        ...
        '설비를 늘어놓거나 물량을 쌓아 두기에 바닥이 넉넉한 규모입니다.',
        '한 공간에서 생산과 보관을 함께 두려는 쪽에서 찾는 크기입니다.',
      ])
      : 고르기([
        '소규모 제조나 보관·소분 작업을 두기에 알맞은 규모입니다.',
        '작업대를 두고 소량을 다루거나 물건을 보관하기에 맞는 크기입니다.',
        '큰 설비 없이 가공·포장·보관을 두려는 쪽에 어울립니다.',
      ])
  }
  if (p.floor?.startsWith('-')) return 고르기([
    '간판 노출보다 면적과 임대료가 중요한 업종에 어울리는 자리입니다.',
    '눈에 띄는 자리보다 넓이와 조건을 먼저 보시는 쪽에 맞습니다.',
    '보관이나 작업 공간처럼 노출이 덜 중요한 용도로 쓰기 좋습니다.',
  ])
  if ((mainAreaM2(p) ?? 0) >= 330) return 고르기([
    '한 층을 통으로 쓰는 업종이나 매장·사무를 함께 두는 쪽에 맞습니다.',
    '공간을 나눠 여러 용도로 쓰거나 한 번에 넓게 쓰려는 쪽에 맞습니다.',
    '인원이 많거나 구획이 여럿 필요한 업종에서 찾는 규모입니다.',
  ])
  if (p.floor === '1') return 고르기([
    '오가는 사람을 바로 받는 소매·식음 업종에 어울리는 1층 자리입니다.',
    '지나가다 들르는 손님을 받기 좋은 1층입니다. 소매나 식음 업종에서 많이 찾습니다.',
    '간판과 출입구가 바로 보이는 1층이라 소매·식음 쪽에서 선호하는 구조입니다.',
    '계단을 오르내리지 않아도 되는 1층이라 방문이 잦은 업종에 맞습니다.',
  ])
  return 고르기([
    '목적을 두고 찾아오는 학원·사무실·서비스업 쪽에 어울리는 자리입니다.',
    '찾아오는 손님을 상대하는 학원·사무실·상담 업종에 맞는 층입니다.',
    '지나는 사람보다 예약이나 문의로 오는 손님이 많은 업종에 어울립니다.',
    '임대료 대비 면적을 먼저 보시는 사무·교육 업종에서 찾는 조건입니다.',
  ])
}
...
function buildDetails(p: ParsedListing, src = ''): string {
  ...
  const loc = spots.length
    ...
    ? `**입지**\n${spots.join(', ')} 조건을 갖춘 자리입니다. `
      + 골라쓰기([
        ...
        '주변에 어떤 가게가 들어와 있는지는 현장에서 같이 돌아보며 설명드립니다.',
        '요일과 시간대에 따라 체감이 다르므로, 편하신 때로 맞춰 안내드리겠습니다.',
      ], seed, 3)
    : null
  ...
  // **주용도가 주택인 상가는 그것을 말해야 한다.**
  //
  // 다가구주택은 건축법상 **단독주택**에 속해, 1층을 점포로 쓰는 자리도
  // 대장 주용도는 `단독주택` 으로 찍힌다. 아파트 단지내상가는 `공동주택` 이다.
  // 등록매물 10건이 그렇다.
  //
  // 그렇다고 "용도변경이 필요합니다" 라고 단정하면 안 된다 — 그 호실만
  // 근린생활시설로 잡혀 있는 경우가 많고, 우리는 건물 주용도만 받았다.
  // **모르는 것은 모른다고 쓰고, 확인해 드리겠다고 말한다.**
  const 주택용도 = /주택|다가구|연립|다세대/.test(p.propertyKind ?? '')
    && p.category !== 'residential' && p.category !== 'land'
  const 인허가 = 주택용도 ? 골라쓰기([
    `건축물대장 주용도는 ${조사(p.propertyKind ?? '', '으로', '로')} 돼 있습니다. 상가로 쓰는 부분의 용도가 따로 잡혀 있는지, 생각하시는 업종의 영업신고가 되는지 계약 전에 대장으로 확인해 드립니다.`,
    `이 건물은 대장 주용도가 ${조사(p.propertyKind ?? '', '이라', '라')} 상가 부분의 용도를 따로 봐야 합니다. 어떤 업종을 보고 계신지 말씀해 주시면 되는 자리인지 대장으로 확인해 드리겠습니다.`,
  ], seed, 1) : 골라쓰기([
    ...
    '어떤 업종을 보고 계신지 말씀해 주시면 이 건물에서 가능한지 확인해 드립니다.',
    '보건증·영업신고가 되는 자리인지는 관심 있는 업종을 알려주시면 알아봐 드립니다.',
    '같은 건물에 어떤 업종이 들어와 있는지도 같이 보셔야 해서, 현장에서 설명드립니다.',
  ], seed, 1)
  // 업종 목록만 나열하면 상가는 126건이 같은 줄로 시작했다. 서두를 바꾼다.
  ...
  // 서류처럼 딱딱해진다. 그 값이 세입자에게 무슨 뜻인지를 문장으로 쓴다.
  const condBits: string[] = []
  condBits.push(p.premium === '없음'
    ? 골라쓰기([
      '권리금 없이 나온 자리라, 처음 들어가는 투자금은 보증금과 시설 공사 정도로 잡으시면 됩니다',
      '무권리 매물이어서 초기 부담이 그만큼 가볍습니다. 남는 예산을 인테리어 쪽으로 쓰시는 분이 많습니다',
    ...
    : p.premium === '유선 문의' ? 골라쓰기([
      ...
      '권리금은 남기는 시설을 어디까지 보느냐에 따라 갈립니다. 전화 주시면 지금 제시된 조건을 알려드립니다',
      '권리금 부분은 전화로만 안내드리고 있습니다. 어느 선까지 이야기되는지도 함께 말씀드립니다',
      '권리금은 시설 범위와 묶여 있어 글로 적기가 어렵습니다. 통화로 정리해 드리겠습니다',
    ], seed, 6)
  ...
  if (p.moveIn === '즉시입주') {
    condBits.push(골라쓰기([
      ...
      '지금 비어 있는 상태라 계약 일정이 곧 입주 일정이 됩니다',
      '앞사람이 나가기를 기다릴 필요가 없어 공사 일정을 먼저 잡으실 수 있습니다',
      '비어 있어 언제부터 쓰실지는 계약하시는 날에 맞추시면 됩니다',
    ], seed, 7))
  } else if (p.moveIn === '협의') {
    condBits.push(골라쓰기([
      '입주 시기는 임대인 사정과 맞물려 있어 언제쯤 필요하신지 말씀해 주시면 맞춰 보겠습니다',
      '입주 날짜는 조율이 필요한 부분입니다. 원하시는 시기를 알려주시면 여쭤보겠습니다',
      '입주는 일정 협의가 되는 매물이라, 계획하신 오픈 시기에 맞출 수 있는지 확인해 드리겠습니다',
    ], seed, 7))
  } else if (p.moveIn) {
    condBits.push(골라쓰기([
      `${p.moveIn}부터 들어가실 수 있습니다`,
      `입주는 ${p.moveIn} 이후로 잡으시면 됩니다`,
    ], seed, 7))
  }
  condBits.push(p.dealType === '매매'
    ? 골라쓰기([
      '나머지 조건은 매도인 사정까지 정리해서 말씀드리겠습니다',
      '잔금 일정이나 승계 범위는 협의할 여지가 있으니 편하게 물어보세요',
      '보고 계신 조건이 있으면 말씀해 주세요. 맞출 수 있는 선인지 확인해 드립니다',
    ], seed, 2)
    : 골라쓰기([
      '그 밖의 조건은 협의 범위가 있으니 편하게 물어보세요',
      '원하시는 선을 말씀해 주시면 임대인과 조율해 보겠습니다',
      '계약 기간이나 렌트프리는 조율이 되는 부분이라 상담 때 함께 봐 드립니다',
      '전화 주시면 임대인이 어디까지 생각하고 있는지도 정리해서 알려드리겠습니다',
      '지금 조건에서 뭐가 걸리시는지 말씀해 주시면 그 부분을 가지고 여쭤보겠습니다',
      '입주 시기나 장비 설치 같은 것도 함께 협의할 수 있는 부분입니다',
    ], seed, 2))
...
function buildQnA(p: ParsedListing): string {
  // 답변을 한 벌로 두면 조건이 비슷한 매물끼리 문답 세 개가 통째로 같아진다.
  // 하루치 10건 중 아홉 건이 같은 답을 달고 나간 적이 있다.
  const s = seedOf(p)
  const pool: Array<[string, string]> = []
  if (!p.maintenanceFee) pool.push([골라쓰기(['관리비는 얼마나 나오나요?', '관리비는 월에 얼마씩 나가나요?', '관리비에는 무엇이 포함되나요?'], s, 10), 골라쓰기([
    '관리비는 건물 관리규약에 따라 부과되어 정확한 금액과 포함 항목을 확인 후 안내드리겠습니다. 문의 주시면 바로 확인해 드립니다.',
    '관리규약에 따라 부과되는 건물입니다. 최근 실제로 나온 금액과 포함 항목을 확인해 알려드리겠습니다.',
    '건물마다 부과 방식이 달라, 이 건물의 최근 부과 내역을 확인해서 안내해 드리겠습니다.',
  ], s, 0)])
  if (p.premium === '유선 문의') pool.push([골라쓰기(['권리금은 어떻게 되나요?', '권리금이 얼마인가요?', '권리금 조건을 알 수 있을까요?'], s, 11), 골라쓰기([
    '권리금은 유선으로 문의 주시면 조건을 안내드리겠습니다. 협의 범위도 함께 설명드립니다.',
    '권리금 부분은 공개된 조건이 아니라 전화로 안내드리고 있습니다. 어느 선까지 조율되는지도 함께 말씀드립니다.',
    '전화 주시면 현재 제시된 권리금과 시설 범위를 같이 알려드리겠습니다.',
    '권리금은 남기시는 시설에 따라 조정되는 부분입니다. 통화하시면 지금 선을 말씀드립니다.',
    '권리금 여부와 금액은 전화로 안내하고 있습니다. 예산 범위를 말씀해 주셔도 됩니다.',
  ], s, 1)])
  if (p.premium === '없음') pool.push([골라쓰기(['정말 권리금이 없나요?', '무권리가 맞나요?', '권리금 없이 들어갈 수 있나요?'], s, 12), 골라쓰기([
    '네, 무권리 매물입니다. 초기 비용은 보증금과 시설 공사 범위 위주로 계획하시면 됩니다.',
    '무권리로 나온 자리입니다. 들어가는 비용은 보증금과 인테리어 정도로 보시면 됩니다.',
    '권리금 없이 나온 매물입니다. 다만 시설 상태는 현장에서 직접 보고 판단하시는 편이 좋습니다.',
  ], s, 2)])
  if (hasParking(p)) pool.push([골라쓰기(['주차는 충분한가요?', '손님 차를 대달 곳이 있나요?', '주차는 몇 대나 되나요?', '차를 댈 자리가 있나요?', '주차 여건은 어떤가요?'], s, 13), 골라쓰기([
    `${parkingLabel(p.parking)} 주차가 가능합니다. 이용 방식(지정/공용)은 현장에서 함께 확인해 드립니다.`,
    `${parkingLabel(p.parking)}까지 주차됩니다. 손님 차를 어디에 대게 할지도 현장에서 같이 봐 드리겠습니다.`,
    `주차는 ${parkingLabel(p.parking)} 가능하며, 지정인지 공용인지는 관리 주체에 확인해 알려드립니다.`,
  ], s, 3)])
  if ((mainAreaM2(p) ?? 0) >= 330) pool.push([골라쓰기(['일부만 임대도 가능한가요?', '나눠서 쓰는 것도 되나요?', '필요한 평수만 쓸 수 있나요?'], s, 14), 골라쓰기([
    '분할 임대 가능 여부는 임대인과 협의가 필요한 부분입니다. 원하시는 면적을 말씀해 주시면 협의해 보겠습니다.',
    '필요하신 면적을 알려주시면 임대인에게 분할이 되는지 확인해 보겠습니다.',
    '통으로 나온 자리라 분할은 협의 사항입니다. 쓰실 평수를 말씀해 주시면 여쭤보겠습니다.',
  ], s, 4)])
  if (p.moveIn === '즉시입주') pool.push([골라쓰기(['입주는 언제부터 가능한가요?', '바로 들어갈 수 있나요?', '언제부터 쓸 수 있나요?', '지금 들어갈 수 있나요?', '입주 시기는 어떻게 되나요?'], s, 15), 골라쓰기([
  ...
  pool.push([골라쓰기(['현장은 언제 볼 수 있나요?', '언제 보러 갈 수 있을까요?', '직접 보고 싶은데 가능한가요?', '한번 보고 싶은데 언제 가능할까요?', '방문해서 볼 수 있나요?'], s, 16), 골라쓰기([
    '연락 주시면 일정을 맞춰 현장 안내드리겠습니다. 방문 전 원하시는 조건을 말씀해 주시면 비교 매물도 함께 준비해 드립니다.',
    '평일·주말 관계없이 시간 맞춰 안내드립니다. 오시기 전에 조건을 말씀해 주시면 비슷한 자리도 같이 보여드리겠습니다.',
    '미리 연락 주시면 임차인 영업에 지장 없는 시간으로 잡아 안내드리겠습니다.',
    '오시는 날짜만 알려주시면 열쇠를 받아 두겠습니다. 내부까지 보셔야 판단이 됩니다.',
    '시간대별로 분위기가 달라서, 생각하시는 영업 시간에 맞춰 안내드리는 편이 좋습니다.',
  ], s, 6)])
  pool.push([골라쓰기(['계약 시 확인해야 할 사항이 있나요?', '계약 전에 뭐를 봐야 하나요?', '서류는 어떻게 확인하나요?', '계약 전에 확인할 게 있을까요?', '등기부는 볼 수 있나요?'], s, 17), 골라쓰기([
    '등기사항과 건축물대장 등 공부 서류는 계약 전에 함께 확인해 드립니다. 궁금하신 부분은 편하게 문의해 주세요.',
    '등기부와 건축물대장을 계약 전에 같이 보면서 설명드립니다. 확인하고 싶으신 항목이 있으면 미리 말씀해 주세요.',
    '소유 관계와 대장상 용도는 계약 전에 서류로 확인해 드립니다. 임대인 사정도 파악되는 대로 말씀드립니다.',
    '등기부를 발급받아 권리 관계를 함께 보면서 설명드립니다. 보증금 안전이 먼저입니다.',
    '건축물대장의 용도와 사용승인일, 위반건축물 여부를 계약 전에 확인해 드립니다.',
  ], s, 7)])

  // 어느 문답이 앞에 오는지도 매물마다 다르게 둔다. 순서가 고정이면 조건이
...
function buildReport(p: ParsedListing, src: string, listingNo: string): string | null {
  ...
  // 공급 = 전용은 **원래 그런 자리가 있다.** 한 층을 통으로 쓰거나 단독 건물이면
  // 나눠 쓸 공용부가 없어 두 값이 같게 나온다. 그걸 다 알리니 등록매물 237건 중
  // 118건에 같은 말이 붙어, 점검 칸이 배경화면이 됐다.
  //
  // **다만 "단층이니까" 만으로는 봐주지 않는다.** 총층이 1이어도 `1389 101호`,
  // `1318 1층 일부`, `제1층 중 1호` 처럼 여러 호실이 나눠 쓰면 공용부가 있다.
  // 호수나 '일부' 가 적혀 있으면 통으로 쓰는 자리가 아니다.
  const 자리 = `${p.addressRaw ?? ''}`
  const 한호실 = /\d+\s*호|일부|중\s*\d+/.test(자리)
  const 통으로쓰는자리 = !한호실
    && (p.totalFloors === '1' || /전체|전층|통임대|단독/.test(`${자리} ${src}`))
