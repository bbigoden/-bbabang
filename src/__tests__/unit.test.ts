...
describe('mapRoomType (19종)', () => {
  ...
  // 비주거 12종 (8db8601 재편: 상가 계열 '(상가)' 접미 통일)
  it('업무(상가)', () => expect(mapRoomType('업무시설')).toBe('업무(상가)'))
  it('1종(상가) - 근린생활', () => expect(mapRoomType('제1종근린생활시설')).toBe('1종(상가)'))
  it('2종(상가) - 근린생활', () => expect(mapRoomType('제2종근린생활시설')).toBe('2종(상가)'))
  it('교육(상가)', () => expect(mapRoomType('교육연구시설')).toBe('교육(상가)'))
  it('공장/창고 - 공장', () => expect(mapRoomType('공장')).toBe('공장/창고'))
  it('공장/창고 - 위험물', () => expect(mapRoomType('위험물 저장 및 처리 시설')).toBe('공장/창고'))
  it('숙박', () => expect(mapRoomType('숙박시설')).toBe('숙박'))
  it('의료(상가)', () => expect(mapRoomType('의료시설')).toBe('의료(상가)'))
  it('운동(상가)', () => expect(mapRoomType('운동시설')).toBe('운동(상가)'))
  it('판매(상가)', () => expect(mapRoomType('판매시설')).toBe('판매(상가)'))
  it('농업/축사', () => expect(mapRoomType('동물 및 식물 관련 시설')).toBe('농업/축사'))
  // 매핑 안 되는 것 → '기타' (위락/자동차/노유자/종교/묘지 등)
  it('기타 - 위락', () => expect(mapRoomType('위락시설')).toBe('기타'))
  it('기타 - 자동차', () => expect(mapRoomType('자동차 관련 시설')).toBe('기타'))
  it('기타 - 노유자', () => expect(mapRoomType('노유자시설')).toBe('기타'))
