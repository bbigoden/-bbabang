      ...
      {open && (
        <div className="rounded-xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-xl overflow-hidden" style={popStyle}>
          ...
          <div className="max-h-64 overflow-y-auto py-1">
            {rows.map(c => (
              <div key={c.key}
                className={`flex items-center justify-between px-3 py-1.5 bg-white hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800 ${c.fixed ? 'cursor-default' : 'cursor-pointer'}`}
...
export default function BrokerCustomersPage() {
  ...
  const init = async () => {
    ...
    // 스코프는 office_broker_id 단일 컬럼 — 직원 목록 조립 왕복이 없다.
    ...
    const custQuery = (from: number, to: number) => supabase.from('broker_customers').select('*')
      .eq('office_broker_id', office).order('received_date', { ascending: false }).order('created_at', { ascending: false })
  ...
  const saveField = useCallback(async (id: string, field: string, value: any) => {
    ...
    if (error) {
      ...
      void findDuplicateCustomers(supabase, officeIdRef.current, value, id).then(dups => {
