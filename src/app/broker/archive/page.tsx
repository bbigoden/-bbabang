        ...
        {authors.length > 0 && (
          <div className="mb-4 flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => setFilterAuthor('all')}
              className={`rounded-full px-3 py-1 text-xs font-semibold transition-colors ${filterAuthor === 'all' ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
              전체
            </button>
            {authors.map(a => (
              <button key={a} onClick={() => setFilterAuthor(a)}
                className={`rounded-full px-3 py-1 text-xs font-semibold transition-colors ${filterAuthor === a ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                {a}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {filteredDiaries.map(d => {
              ...
              return (
                <li key={d.id} className="rounded-xl border border-gray-200 bg-white overflow-hidden">
                  <button onClick={() => setOpenId(isOpen ? null : d.id)}
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        작성자 <span className="font-semibold text-gray-700">{d.author_name || '—'}</span>
                        {dcusts.length > 0 && (
