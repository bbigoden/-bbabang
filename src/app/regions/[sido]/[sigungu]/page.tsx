    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        <p className="mt-2 text-sm text-gray-500">
          활성 요청 <span className="font-bold text-gray-800">{requestsArr.length}</span>건 · 관심 등록 중개사 <span className="font-bold text-gray-800">{brokerCount ?? 0}</span>명
        </p>
                ...
                <li key={b.id}>
                  <Link href={`/broker/${b.id}`}
                    ...
                    <div className="flex items-center gap-1.5 mb-1">
                      <p className="font-bold text-gray-900 truncate flex-1">{b.user_name ?? '(이름 없음)'}</p>
                  ...
                  return (
                    <li key={dt}>
                      <div className="flex items-center justify-between text-sm mb-1">
                        <span className="font-semibold text-gray-700">{dt}</span>
                        <span className="text-gray-500">{count}건 · <span className="font-bold text-gray-800">{pct}%</span></span>
                      </div>
        ...
        {dongs.length > 0 && (
          <section className="mt-10">
            <h2 className="mb-3 text-lg font-bold text-gray-900">동·읍·면별 요청</h2>
        ...
        {/* 최근 요청 */}
        <section className="mt-10">
          <h2 className="mb-3 text-lg font-bold text-gray-900">{sigunguN} 최근 요청</h2>
        ...
        <section className="mt-12 rounded-2xl border border-gray-200 bg-white p-6">
          ...
          <dl className="space-y-4 text-sm">
            <div>
              <dt className="font-semibold text-gray-800">Q. {sigunguN}에서 부동산 매물 어떻게 찾나요?</dt>
              <dd className="mt-1 text-gray-500">A. 빠방에서 조건만 등록하면 인증된 공인중개사가 직접 매물을 제안해드려요. 가입·요청 등록 모두 무료입니다.</dd>
            </div>
            <div>
              <dt className="font-semibold text-gray-800">Q. {sigunguN}에 등록된 중개사는 몇 명인가요?</dt>
              <dd className="mt-1 text-gray-500">A. 현재 {brokerCount ?? 0}명이 이 지역을 담당하고 있습니다.</dd>
            </div>
            {dealTypes.length > 0 && (
              <div>
                <dt className="font-semibold text-gray-800">Q. {sigunguN}에서 가장 많이 요청되는 거래는?</dt>
