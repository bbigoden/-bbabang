        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-gray-200 bg-white py-16 text-center">
            ...
            <div className="mt-5 flex justify-center gap-2">
              ...
              <Link href="/explore/requests" className="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">
                요청 둘러보기
              </Link>
            </div>
          </div>
          ...
          <ul className="space-y-3">
            {items.map(s => {
              ...
              return (
                <li key={s.id} className="rounded-2xl border border-gray-200 bg-white p-4 hover:border-blue-300 transition-colors">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-base font-bold text-gray-900">{s.label || '(이름 없음)'}</p>

                      {/* 필터 칩 */}
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {Object.entries(s.filters).filter(([_, v]) => v !== null && v !== undefined && v !== '' && v !== false).map(([k, v]) => (
                          <span key={k} className="inline-flex items-center gap-1 rounded-md bg-gray-50 border border-gray-200 px-2 py-0.5 text-[11px] font-medium text-gray-700">
                    ...
                    <div className="flex flex-col gap-1 flex-shrink-0">
                      ...
                      <button onClick={() => remove(s.id)}
                        className="inline-flex items-center justify-center gap-1 rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-500 hover:bg-red-50 hover:text-red-500 hover:border-red-200">
